...
#             vocab_file=vocab_file, do_lower_case=do_lower_case)
        ...
        if data_file != None:
#             self.raw_data = open(data_file, 'r').readlines()
            self.raw_data = pd.read_csv(data_file,sep='\t').dropna()
...
#         tokens = self.tokenizer.tokenize(row[1])
#         tokens = jieba.lcut(line['content'])
        tokens = jieba.lcut(line['text'])
        tokens = tokens[:(self.max_seq_len - 1)]
        tokens = ["[CLS]"] + tokens
        tokens = self.tokenizer.convert_tokens_to_ids(tokens)
        return {"text":line['text'], "tokens" : tokens, "label" : label}
